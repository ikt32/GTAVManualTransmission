...
// only saves [OPTIONS] and [FORCE_FEEDBACK] as the other stuff SHOULD be generated by
// my tool thingywingy
void ScriptSettings::SaveWheel() const {
	CSimpleIniA settingWheel;
	settingWheel.SetUnicode();
	settingWheel.LoadFile(settingsWheelFile.c_str());

	// [OPTIONS]
	settingWheel.SetBoolValue("OPTIONS", "EnableWheel", EnableWheel);
	settingWheel.SetBoolValue("OPTIONS", "WheelWithoutManual", WheelWithoutManual);
	settingWheel.SetBoolValue("OPTIONS", "WheelBoatPlanes", AltControls);
	settingWheel.SetBoolValue("OPTIONS", "PatchSteering", PatchSteering);
